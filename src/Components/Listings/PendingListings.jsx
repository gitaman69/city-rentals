...
const PendingListings = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      const fetchPendingListings = async () => {
        try {
          const response = await fetch('https://backend-server-orcin.vercel.app/api/pending-listings');
          if (!response.ok) {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await fetch('https://backend-server-orcin.vercel.app/api/approve-listing', {
        method: 'POST',
        headers: {
  ...
  const handleReject = async (id) => {
    try {
      const response = await fetch('https://backend-server-orcin.vercel.app/api/reject-listing', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
